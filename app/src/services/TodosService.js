...
const TodosServices = class {
	...
	getSearchTodos(query) {
		return axios.get(`/todos?q=${query}`).then(({ data }) => data);
	}
	getTodo(id) {
		return axios.get(`/todos/${id}`).then(({ data }) => data);
	}
	postTodo(todo) {
		return axios.post(`/todos`, todo).then(({ data }) => data);
	}
	putTodo(todo) {
		return axios.put(`/todos/${todo.id}`, todo).then(({ data }) => data);
	}
	deleteTodo(id) {
